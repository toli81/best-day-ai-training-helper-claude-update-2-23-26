...
const ExerciseLibrary: React.FC<ExerciseLibraryProps> = ({ 
  ...
  // Exercise Name Editing state
  const [editingExerciseId, setEditingExerciseId] = useState<string | null>(null);
  const [newExerciseName, setNewExerciseName] = useState('');

  // Flatten all exercises with their parent session context
  ...
  const uniqueTags = useMemo(() => {
    ...
    allExercises.forEach(ex => ex.tags.forEach(tag => tags.add(tag)));
  ...
  const handleSaveEditTag = () => {
    if (editingTag && newTagName && editingTag !== newTagName) {
      onUpdateGlobalTag(editingTag, newTagName);
    }
    setEditingTag(null);
  };
  ...
  const handleStartEditExerciseName = (ex: any) => {
  ...
  const handleSaveExerciseName = (sessionId: string, exerciseId: string) => {
    if (newExerciseName.trim()) {
      onUpdateExerciseName(sessionId, exerciseId, newExerciseName.trim());
    }
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">
            Aggregated technical data from {allExercises.length} recorded sets
          </p>
        </div>
        <button 
      ...
      {isManagingTags && (
        <div className="bg-slate-900 p-8 rounded-[40px] shadow-2xl space-y-6 animate-in slide-in-from-top-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div key={tag} className="flex items-center justify-between bg-slate-800 p-4 rounded-2xl group">
                {editingTag === tag ? (
                  <div className="flex items-center gap-2 w-full">
                    <input 
                      autoFocus
                      className="bg-slate-700 text-white text-xs px-3 py-1.5 rounded-lg border-none focus:ring-2 focus:ring-brand-500 flex-grow"
                      value={newTagName}
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        ...
                        className="p-2 text-slate-400 hover:text-brand-400 hover:bg-slate-700 rounded-lg"
                        title="Rename Global Tag"
                      >
                      ...
                      <button 
                        onClick={() => window.confirm(`Remove "${tag}" from ALL exercises?`) && onDeleteGlobalTag(tag)}
            ...
            {uniqueTags.length === 0 && (
              <p className="text-slate-600 text-[10px] font-black uppercase tracking-widest col-span-full py-4 text-center">No tags yet. Tag an exercise below.</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm space-y-8">
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 relative">
            ...
            <input 
              type="text" 
              placeholder="Search by exercise name or athlete..." 
              value={search}
          ...
          <div key={`${ex.sessionId}-${ex.id}`} className="group bg-white border border-slate-200 rounded-[32px] overflow-hidden hover:border-brand-200 hover:shadow-2xl transition-all duration-500 flex flex-col">
            <div className="p-6 flex-grow space-y-4">
              <div className="flex justify-between items-start">
                <div className="flex-grow pr-4">
                  {editingExerciseId === ex.id ? (
                    ...
                    <div className="flex items-center gap-2 group/title">
                      <h4 className="font-black text-slate-900 uppercase tracking-tight text-lg group-hover:text-brand-500 transition-colors cursor-pointer" onClick={() => handleStartEditExerciseName(ex)}>
                        {ex.name}
                      </h4>
                      <button 
                        onClick={() => handleStartEditExerciseName(ex)}
                ...
                <div className="bg-brand-50 text-brand-500 text-[10px] font-black px-2 py-1 rounded-lg uppercase whitespace-nowrap">
                  {ex.reps} Reps
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {ex.tags.map(tag => (
                  <span 
                    key={tag} 
                    className="group/tag text-[7px] bg-slate-50 text-slate-400 px-2 py-0.5 rounded-full uppercase font-black tracking-widest border border-slate-100 flex items-center gap-1.5 hover:bg-brand-50 hover:text-brand-500 transition-colors"
                  >
                    {tag}
                    <button 
                      onClick={() => onRemoveTagFromExercise(ex.sessionId, ex.id, tag)}
              ...
              <div className="bg-slate-50 p-4 rounded-2xl">
                <p className="text-[10px] font-bold text-slate-500 italic leading-relaxed">
                  "{ex.cues[0] || 'High intensity performance'}"
                </p>
              </div>
              ...
              <div className="pt-2 flex items-center gap-2">
                ...
                />
                <button 
                  onClick={() => handleAddTag(ex.sessionId, ex.id)}
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
              ...
              <button 
                onClick={() => onViewExercise(ex.sessionId, ex)}
      ...
      {filteredExercises.length === 0 && (
        <div className="py-20 text-center bg-white border-2 border-dashed border-slate-200 rounded-[40px]">
          <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No matching exercises in your library</p>
        </div>
      )}
    </div>
  );
